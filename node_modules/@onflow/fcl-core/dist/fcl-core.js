...
async function pre(context, type, opts) {
  ...
  // prettier-ignore
  utilInvariant.invariant(await context.config.get("accessNode.api"), `${type}(opts) -- Required value for "accessNode.api" not defined in config. See: ${"https://github.com/onflow/flow-js-sdk/blob/master/packages/fcl/src/exec/query.md#configuration"}`);
}
...
async function deriveCadenceByNetwork110(_ref) {
  ...
  utilInvariant.invariant(template.f_version === "1.1.0", "deriveCadenceByNetwork110({ template }) -- template must be version 1.1.0");
...
async function fetchChainId(context) {
  ...
  const response = await context.sdk.send([sdk.getNetworkParameters()], opts).then(context.sdk.decode);
...
function createQueryRaw(context) {
  ...
  async function queryRaw() {
    ...
    return context.sdk.send([sdk__namespace.script(opts.cadence), sdk__namespace.args(normalizeArgs(opts.args || [])), sdk__namespace.atLatestBlock(opts.isSealed ?? false), opts.limit && typeof opts.limit === "number" && sdk__namespace.limit(opts.limit)]);
  }
  return queryRaw;
}
...
class TransactionError extends Error {
  constructor(message, code) {
    super(message);
    this.code = code;
    this.type = typedefs.FvmErrorCode[code];
  }
...
function createTransaction(context) {
  ...
  transaction.isUnknown = sdk.isUnknown;
...
function createObservable(context, txId, opts) {
  ...
  let value = {
    blockId: "",
    status: typedefs.TransactionExecutionStatus.UNKNOWN,
    statusCode: 0,
    errorMessage: "",
    events: [],
    statusString: ""
  ...
  function subscribeTransactionStatuses() {
    ...
    const subscription = context.sdk.subscribe({
      topic: typedefs.SubscriptionTopic.TRANSACTION_STATUSES,
      args: {
        transactionId: txId
      },
      onData: txStatus => {
      ...
      onError: err => {
        if (err instanceof sdk.SubscriptionsNotSupportedError) {
    ...
    [TICK]: async ctx => {
      ...
      if (hwm == null) {
        ...
        if (hwm.height < next.height) {
          const data = await fclContext.sdk.send([sdk.getEventsAtBlockHeightRange(ctx.self(), hwm.height + 1, next.height)]).then(fclContext.sdk.decode);
...
const spawnEvents = (fclContext, key) => utilActor.spawn(createHandlers$1(fclContext), key);
function createLegacyEvents(context) {
  ...
  function legacyEvents(key) {
    return {
       ...
       */
      subscribe: callback => utilActor.subscriber(key, address => spawnEvents(context, address), callback)
    };
  }
  return legacyEvents;
}
      ...
      subscribe: function (onData) {
        ...
        function subscribeEventStream() {
          const {
            ...
            topic: typedefs.SubscriptionTopic.EVENTS,
            args: filter,
            onData: event => {
              // Emit the event
              onData(event);
            ...
            onError: error => {
              ...
              if (error instanceof sdk.SubscriptionsNotSupportedError) {
...
const spawnProviders = context => utilActor.spawn(createHandlers(context), SERVICE_ACTOR_KEYS.AUTHN);
...
function createAuthn(context) {
  ...
  const authn = {
     ...
     * unsubscribe();
     */
    subscribe: cb => utilActor.subscriber(SERVICE_ACTOR_KEYS.AUTHN, () => spawnProviders(context), cb),
    /**
     ...
     * }
     */
    snapshot: () => utilActor.snapshoter(SERVICE_ACTOR_KEYS.AUTHN, () => spawnProviders(context)),
    /**
...
async function generateTemplateId$2(_ref) {
  ...
  utilInvariant.invariant(template.f_version === "1.0.0", "generateTemplateId({ template }) -- template object must be an version 1.0.0");
...
function createGenerateDependencyPin110(context) {
  ...
  async function generateDependencyPin110(_ref) {
    ...
    for (const horizonImport of horizon) {
      const account = await context.sdk.send([sdk.getAccount(await context.config.get(horizonImport.address, horizonImport.address))], opts).then(context.sdk.decode);
...
function getTemplateMessage(_ref) {
  ...
  utilInvariant.invariant(messageKey, "getTemplateMessage({ messageKey }) -- messageKey must be defined");
  utilInvariant.invariant(typeof messageKey === "string", "getTemplateMessage({ messageKey }) -- messageKey must be a string");
  utilInvariant.invariant(localization, "getTemplateMessage({ localization }) -- localization must be defined");
  utilInvariant.invariant(typeof localization === "string", "getTemplateMessage({ localization }) -- localization must be a string");
  utilInvariant.invariant(template != undefined, "getTemplateMessage({ template }) -- template must be defined");
  utilInvariant.invariant(typeof template === "object", "getTemplateMessage({ template }) -- template must be an object");
  utilInvariant.invariant(template.f_type === "InteractionTemplate", "getTemplateMessage({ template }) -- template object must be an InteractionTemplate");
  switch (template.f_version) {
    case "1.1.0":
...
function getTemplateArgumentMessage(_ref) {
  ...
  sdk.invariant(typeof argumentLabel === "string", "getTemplateArgumentMessage({ argumentLabel }) -- argumentLabel must be a string");
  ...
  sdk.invariant(template.f_type === "InteractionTemplate", "getTemplateArgumentMessage({ template }) -- template object must be an InteractionTemplate");
  switch (template.f_version) {
    case "1.1.0":
...
const createMutate = context => {
  ...
  const mutate = async function () {
    ...
    try {
      ...
      txid = context.sdk.send([sdk__namespace.transaction(opts.cadence), sdk__namespace.args(normalizeArgs(opts.args || [])), opts.limit && isNumber(opts.limit) && sdk__namespace.limit(opts.limit),
      // opts.proposer > opts.authz > authz
      sdk__namespace.proposer(opts.proposer || opts.authz || authz),
      // opts.payer > opts.authz > authz
      sdk__namespace.payer(opts.payer || opts.authz || authz),
      // opts.authorizations > [opts.authz > authz]
      sdk__namespace.authorizations(opts.authorizations || [opts.authz || authz])]).then(context.sdk.decode);
      return txid;
    } catch (error) {
      throw error;
    }
...
const spawnCurrentUser = context => {
  utilActor.spawn(makeHandlers(context), context.actorName);
};
...
async function getAccountProofData(context) {
  ...
  if (accountProofData.appIdentifier) {
    ...
    utilInvariant.invariant(origin, "The appIdentifier (origin) could not be inferred from the window.location.origin.  Please set the appIdentifier manually in the fcl.accountProof.resolver function.");
    ...
    if (user.loggedIn && !forceReauth) {
      if (refreshService) {
        try {
          ...
          utilActor.send(context.actorName, SET_CURRENT_USER, await buildUser(response));
        } catch (error) {
          logger.log({
...
function createUnauthenticate(context) {
  ...
  return function unauthenticate() {
    spawnCurrentUser(context);
    utilActor.send(context.actorName, DEL_CURRENT_USER);
  };
}
...
function createSubscribe(context) {
  ...
  return function subscribe(callback) {
    ...
    const self = utilActor.spawn(async ctx => {
      ctx.send(context.actorName, utilActor.SUBSCRIBE);
      while (1) {
        const letter = await ctx.receive();
        if (letter.tag === EXIT) {
          ctx.send(context.actorName, utilActor.UNSUBSCRIBE);
          return;
        }
...
function createSnapshot(context) {
  ...
  return function snapshot() {
    spawnCurrentUser(context);
    return utilActor.send(context.actorName, SNAPSHOT, null, {
      expectReply: true,
      timeout: 0
    });
  };
...
const createResolveArgument = config => async () => {
  ...
  return sdk.arg(utilAddress.withPrefix(addr), sdk.t.Address);
};
...
const getCurrentUser = cfg => {
  ...
  const getStorageProvider = async () => {
    ...
    return await config.config.first(["fcl.storage", "fcl.storage.default"], undefined);
  };
  return _createUser({
    ...partialContext,
    getStorageProvider,
 ...
 */
function createFCLContext(config$1) {
  let contracts;
  if (config$1.flowJson) {
    utilInvariant.invariant(!!config$1.flowNetwork, "If flowJson is provided, flowNetwork must also be specified.");
    const cleanedNetwork = config$1.flowNetwork.toLowerCase().replace(/^local$/, "emulator");
    utilInvariant.invariant(cleanedNetwork === "mainnet" || cleanedNetwork === "testnet" || cleanedNetwork === "emulator", `Invalid flowNetwork: ${config$1.flowNetwork}. Must be one of: mainnet, testnet, emulator.`);
    contracts = config.getContracts(config$1.flowJson, cleanedNetwork);
  }
  const sdk$1 = sdk.createSdkClient({
    accessNodeUrl: config$1.accessNodeUrl,
    transport: config$1.transport,
    computeLimit: config$1.computeLimit,
    customResolver: config$1.customResolver,
    customDecoders: config$1.customDecoders,
    contracts: contracts
  });
  const configService = createConfigService(config$1);
  const currentUser = createUser({
    platform: config$1.platform,
    storage: config$1.storage,
    config: configService,
    discovery: {
      execStrategy: config$1.discovery?.execStrategy
    },
    sdk: sdk$1
  });
  return {
...
function createFlowClientCore(params) {
  const context = createFCLContext({
    ...params,
    transport: params.transport || transportHttp.httpTransport
  });
  return {
    // Global services
...
Object.defineProperty(exports, "flowEmulator", {
  enumerable: true,
  get: function () { return sdk.flowEmulator; }
});
Object.defineProperty(exports, "flowMainnet", {
  enumerable: true,
  get: function () { return sdk.flowMainnet; }
});
Object.defineProperty(exports, "flowTestnet", {
  enumerable: true,
  get: function () { return sdk.flowTestnet; }
});
...
Object.defineProperty(exports, "subscribe", {
  enumerable: true,
  get: function () { return sdk.subscribe; }
});
...
Object.defineProperty(exports, "subscribeRaw", {
  enumerable: true,
  get: function () { return sdk.subscribeRaw; }
});
...
exports.createFlowClientCore = createFlowClientCore;
